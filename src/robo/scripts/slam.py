...
import rospy
from robo.msg import MergedReadings,LocalizationReadings
...
class Map:
    ...
    def get_pose(self, pose: PoseStamped):
        
        x, y, z, w = pose.pose.orientation.x, \
                     pose.pose.orientation.y, \
                     pose.pose.orientation.z, \
                     pose.pose.orientation.w
        _, _, theta = tft.euler_from_quaternion([x, y, z, w])
        px, py = self.xy2ij(pose.pose.position.x,
    ...
    def build_map(self, merged_readings: LocalizationReadings):
        print("Building the map ...")
        laser = merged_readings.laser
        pose = merged_readings.pose

        dists = np.array(laser.ranges) / RVIZ.RESOLUTION

        true_x, true_y, true_theta = self.get_pose(pose)
...
if __name__ == '__main__':
    try:
        rospy.init_node("node_slam")
        rospy.loginfo("node_slam is up and running")
        my_map = Map()
        sensor_sub = rospy.Subscriber("/correct_readings", LocalizationReadings,
                                      map_callback)
